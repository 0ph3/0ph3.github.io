...
We can now build a ```findUsername()``` function to iterate through all ascii alphanumerical characters until we find the full the username.
An empty ```username``` is initially passed into the function and each time the oracle function finds a valid character, we recursively call the function adding the valid character to the current username value until we extract the full username. The current username string is tested each time we find a valid character to determine if it is the full username.
We do this using a standard authentication bypass payload  and current username ```username=<CURRENT-USERNAME>&password[$ne]=invalid``` 
...
The valid username is passed to the oracle function and the ```target_parameter``` is set to ```password``` to change the request data to use our payload to target the password field.
...
![Desktop View](/assets/img/posts/HTB/Mango/Mango.gif){: width="250" height="250" }

>This footage is sped up, the actual runtime was about 5 minutes to extract both users.
{: .prompt-info }

We are able to extract the username and password of two users in the database. 
...
While the the admin credentials don't seem to get us an SSH credential, we are able to connect to the server as the mango user.
```console
...
We can find the ```user.txt``` in their home directory.
```console
...
While going through the rest of the usual post exploitation enumeration steps, we can find the ```jjs``` binary that has the SUID set for the admin group.
This means we can run the binary with root level access.
```console
...
If we check [GTFOBins](https://gtfobins.github.io/gtfobins/) for jjs, we find that the binary essentially allows us to run java code.
We have several options options but the obvious option is to check if ssh is misconfigured to allow root login. If so, we can generate an ssh key-pair and use ```jjs``` SUID privileges to write our public key to the root user's ```authorized_keys``` file, effectively allowing us to authenticated as the root user via ssh using our private key.
...
id_rsa  id_rsa.pub
```
On the server, can create a file named ```exploit``` containing the following java payload to write our ssh key to ```/root/.ssh/authorized_keys```
```java
...
To run the code we can type ```jjs``` followed by the name of our file
```console
...
```
 ...
 We are now root and can retrieve the flag in /root/root.txt!
```console
...
```
>Remember to use ```chmod 600 id_rsa to restric privileges on your private key, otherwise ssh will throw an error when trying to use it```
{: .prompt-info }
