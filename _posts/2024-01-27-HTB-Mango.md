...
While going through the usual post exploitation enumeration steps, we can find a binary that has the SUID set for the admin group.
...
We have several options but the obvious option is to check if ssh is misconfigured to allow root login. If so, we can generate an ssh key-pair and write our public key to the root user's ```authorized_keys``` file, effectively allowing us to authenticated to the root user with our private key.
