...
With unsanitized input from the above query, we can use the ```$ne``` operator to inject a comparison, ```{$ne: "something invalid"}```, causing the search result for the username and password return as ```true```.
```javascript
db.users.find({
...
Armed with an simple understanding of how NoSQLi authentication bypass attacks work, we can intercept the authentication request and try injecting a payload to get us in.
...
We can achieve this by chaning the username and password parameters as ```username[$ne]=invalid```
