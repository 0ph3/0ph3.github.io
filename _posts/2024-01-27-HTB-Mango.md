...
The function extracts the password by passing the valid username to the oracle function while setting the ```target_parameter``` to ```password``` to change the request data the oracle functon will use to inject our payload
